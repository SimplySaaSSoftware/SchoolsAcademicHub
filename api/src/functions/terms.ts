  ...
  try {
    ...
    if (req.method === 'GET') {
      const rows = await sql<{ data: TermDoc }[]>`
        SELECT data FROM items WHERE school_id = ${schoolId} AND type = 'term'
    ...
    if (req.method === 'POST') {
      ...
      const doc: TermDoc = { id: randomUUID(), school_id: schoolId, type: 'term', name: name.trim(), created_at: new Date().toISOString() };
